...
class ModuleStatistiques:
            ...
            if var in df.columns and pd.api.types.is_numeric_dtype(df[var]):
                ...
                if len(data) > 0:
                    results[var] = {
                        ...
                        'asymetrie': round(float(stats.skew(data)), 3),
                        'aplatissement': round(float(stats.kurtosis(data)), 3),
    ...
    def test_normalite(df, variables):
        ...
        for var in variables:
            if var in df.columns and pd.api.types.is_numeric_dtype(df[var]):
                ...
                if 3 <= len(data) <= 5000:  # Shapiro fonctionne entre 3 et 5000 observations
    ...
    def analyse_variance(df, variable, facteur):
        ...
        if len(groups) >= 2:
            ...
            # Post-hoc Tukey si ANOVA significative
            if p_value < 0.05 and len(groups) > 2:
                try:
                    # Méthode simplifiée pour le post-hoc
                    from statsmodels.stats.multicomp import pairwise_tukeyhsd
                    
                    data = df[[variable, facteur]].dropna()
                    tukey = pairwise_tukeyhsd(data[variable], data[facteur])
                    
                    result['tukey'] = str(tukey.summary())
                except:
                    result['tukey'] = "Non disponible"
    ...
    with tab3:
        ...
        if st.button("🔬 Exécuter ANOVA"):
            ...
            if result:
                ...
                if result['significatif'] and 'tukey' in result:
                    st.markdown("#### 📋 TEST POST-HOC (TUKEY)")
    ...
    with tab4:
        ...
        if test_vars:
            ...
            if normalite:
                ...
                selected_var = st.selectbox("Variable pour QQ-Plot", test_vars)
